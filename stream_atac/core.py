...
def preprocess_atac(file_count=None,file_region=None,file_sample=None, genome = None, motif_species = 'Homo sapiens',
    ...
    **kwargs: additional arguments to `Anndata` reading functions
    ...
    if(sum(list(map(lambda x: x is not None,[file_count,file_region,file_sample,genome])))<4):
        print('count file, region file, sample file, and genome should all be provided.')
        return
    else:
